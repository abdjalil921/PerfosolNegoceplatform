...
import {
    CreditCard, Plus, X, Trash2, Download, Printer,
...
function TransactionModal({ onClose, onSave, editData }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.transaction_date) { setError(t('bank.dateRequired')); return; }
        setSaving(true);
        ...
        else setError(result.error || t('bank.saveFailed'));
    };

    const inputClass = "w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400";
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-base font-semibold text-gray-900">
                            {isEdit ? t('bank.editTransaction') : t('bank.addTransaction')}
                        </h2>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className={labelClass}>{t('bank.transactionDate')} *</label>
                            <input type="date" value={form.transaction_date}
                                onChange={e => set('transaction_date', e.target.value)}
                                className={inputClass} />
                        </div>
                        <div>
                            <label className={labelClass}>{t('sales.paymentDate')}{isEdit && editData?.source_id ? ' (auto)' : ''}</label>
                            <input type="date" value={form.payment_date}
                                onChange={e => set('payment_date', e.target.value)}
                                readOnly={isEdit && Boolean(editData?.source_id)}
                                className={`${inputClass} ${isEdit && editData?.source_id ? 'bg-gray-50 text-gray-400 cursor-not-allowed' : ''}`} />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className={labelClass}>{t('bank.libelle')}</label>
                            <input type="text" value={form.libelle}
                                onChange={e => set('libelle', e.target.value)}
                                placeholder={t('bank.libellePlaceholder')}
                                className={inputClass} />
                        </div>
                    </div>
                    ...
                    <div className="flex justify-end gap-3 pt-1">
                        ...
                        <button type="submit" disabled={saving}
                            ...
                            {isEdit ? t('common.save') : t('bank.saveTransaction')}
                        </button>
                    </div>
                </form>
            </div>
...
export default function BankPayments() {
    ...
    const totalSorties = filtered.reduce((s, tx) => s + (Number(tx.sorties) || 0), 0);
    const balance = totalEntrees - totalSorties;
    const balancePositive = balance >= 0;

    // ── CSV Export ──
    const exportCSV = () => {
        const headers = [t('bank.transactionDate'), t('common.receiptNumber'), t('bank.libelle'), t('bank.entrees'), t('bank.sorties'), t('sales.paymentDate')];
        const rows = filtered.map(tx => [
            tx.transaction_date || '', tx.receipt_number || '', tx.libelle || '',
            Number(tx.entrees) || 0, Number(tx.sorties) || 0, tx.payment_date || '',
        ]);
        ...
        a.href = url;
        a.download = `bank_payments_${today()}.csv`;
        a.click();
    ...
    const handlePrint = () => {
        ...
        const rows = filtered.map(tx => `
            <tr>
                ...
                <td>${(tx.receipt_number || '—').replace(/</g, '&lt;')}</td>
                ...
                <td style="text-align:center">${tx.payment_date
                    ? `<span style="background:#f0fdf4;color:#15803d;border:1px solid #bbf7d0;padding:2px 8px;border-radius:99px;font-size:10px;font-weight:700">✓ ${fmtDate(tx.payment_date)}</span>`
                    : `<span style="background:#fffbeb;color:#d97706;border:1px solid #fde68a;padding:2px 8px;border-radius:99px;font-size:10px">Unpaid</span>`
                }</td>
            </tr>`).join('');
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .header{text-align:center;margin-bottom:16px;padding-bottom:12px;border-bottom:2px solid #1d4ed8}
  .header h1{font-size:22px;font-weight:700;color:#1d4ed8;letter-spacing:.01em}
  ...
  th{padding:8px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:#1d4ed8;border-bottom:2px solid #bfdbfe;text-align:left}
  td{padding:7px 10px;border-bottom:1px solid #f0f0f0}
  tr:nth-child(even) td{background:#fafaf9}
  tfoot td{padding:9px 10px;font-weight:700;background:#eff6ff;border-top:2px solid #bfdbfe}
  ...
  <div class="summary">
    <div class="box e"><label>${t('bank.totalEntrees')}</label><div class="amount">+${fmt(totalEntrees)} MAD</div></div>
    <div class="box s"><label>${t('bank.totalSorties')}</label><div class="amount">-${fmt(totalSorties)} MAD</div></div>
    <div class="box ${balance >= 0 ? 'bp' : 'bn'}"><label>${t('bank.balance')} (${balance >= 0 ? t('bank.balancePositive') : t('bank.balanceNegative')})</label><div class="amount" style="color:${balColor}">${balSign}${fmt(balance)} MAD</div></div>
  </div>
  <table>
    <thead><tr>
      <th>${t('bank.transactionDate')}</th>
      <th>${t('common.receiptNumber')}</th>
      <th>${t('bank.libelle')}</th>
      <th style="text-align:right;color:#15803d">${t('bank.entrees')}</th>
      <th style="text-align:right;color:#dc2626">${t('bank.sorties')}</th>
      <th style="text-align:center">${t('sales.paymentDate')}</th>
    </tr></thead>
    <tbody>${rows}</tbody>
    <tfoot><tr>
      <td colspan="3"><strong>${t('bank.balance')}:</strong> <span style="color:${balColor};font-family:monospace">${balSign}${fmt(balance)} MAD</span></td>
        ...
        <div className="space-y-5">
            ...
            {confirmDeleteId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
                    <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm text-center">
                        ...
                        <p className="font-semibold text-gray-900 mb-1">{t('bank.confirmDelete')}</p>
                        <p className="text-sm text-gray-500 mb-5">{t('bank.confirmDeleteDesc')}</p>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">{t('bank.subtitle')}</p>
                </div>
                {!isComptable && (
            ...
            <div className="flex flex-col sm:flex-row gap-2">
                ...
                <div className="relative">
                    ...
                    >
                        ...
                        {t('common.filterBy')}
                        {filterCount > 0 && (
                            <span className="ml-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-blue-700 text-white text-[10px] font-bold">{filterCount}</span>
                        )}
                        ...
                        <div className="absolute right-0 top-full mt-1.5 z-30 bg-white border border-gray-200 rounded-xl shadow-lg p-4 w-72">
                            ...
                            <div className="grid grid-cols-2 gap-1.5 mb-3">
                                <button onClick={() => setSortOrder('desc')}
                                    className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                                        sortOrder === 'desc' ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                                    }`}>
                                    {t('common.sortNewest')}
                                </button>
                                <button onClick={() => setSortOrder('asc')}
                                    className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                                        sortOrder === 'asc' ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                                    }`}>
                                    {t('common.sortOldest')}
                                </button>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-1.5 mb-3">
                                <button onClick={() => setSortField('date')}
                                    className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                                        sortField === 'date' ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                                    }`}>
                                    {t('common.filterByTransactionDate')}
                                </button>
                                <button onClick={() => setSortField('receipt')}
                                    className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                                        sortField === 'receipt' ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                                    }`}>
                            ...
                            <div className="mb-3">
                                ...
                                <div className="grid grid-cols-2 gap-1.5">
                                    <button onClick={() => setDateField('transaction')}
                                        className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                                            dateField === 'transaction' ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                                        }`}>
                                        {t('common.filterByTransactionDate')}
                                    </button>
                                    <button onClick={() => setDateField('payment')}
                                        className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors ${
                                            dateField === 'payment' ? 'bg-blue-700 text-white border-blue-700' : 'bg-white text-gray-600 border-gray-200 hover:bg-gray-50'
                                        }`}>
                            ...
                            <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                                ...
                                <button onClick={() => setShowFilterPanel(false)}
                                    className="px-3 py-1.5 text-xs font-medium text-white bg-blue-700 hover:bg-blue-800 rounded-lg transition-colors">
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-12 text-center">
                    ...
                    <p className="text-sm font-medium text-gray-500">
                        {hasFilters ? t('bank.noResults') : t('bank.noTransactions')}
                    </p>
                    <p className="text-xs text-gray-400 mt-1">
                        {hasFilters ? t('bank.noResultsDesc') : t('bank.noTransactionsDesc')}
                    </p>
                </div>
            ) : (
                <>
                        ...
                        {filtered.map(tx => (
                            <div key={tx.id} className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
                                <div className="flex items-start justify-between">
                                    ...
                                    {!isComptable && !tx.source_id && (
                                        <div className="flex items-center gap-1">
                                            <button onClick={() => setEditingTx(tx)} className="p-1 text-gray-300 hover:text-blue-500 transition-colors">
                                                <Pencil className="w-4 h-4" />
                                            </button>
                                            <button onClick={() => setConfirmDeleteId(tx.id)} className="p-1 text-gray-300 hover:text-red-500 transition-colors">
                                ...
                                <div className="flex items-center justify-between mt-2">
                                    ...
                                    {tx.payment_date
                                        ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-green-50 text-green-700 border border-green-100">✓ Paid · {fmtDate(tx.payment_date)}</span>
                                        : <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-600 border border-amber-100">Unpaid</span>
                                    }
                                </div>
                                <div className="flex gap-6 mt-3 pt-3 border-t border-gray-100">
                                    <div>
                                        <p className="text-xs text-gray-400">{t('bank.entrees')}</p>
                                        <p className="text-sm font-mono font-semibold text-emerald-700">+{fmt(tx.entrees)}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-400">{t('bank.sorties')}</p>
                                    ...
                                    {filtered.map((tx, idx) => {
                                        ...
                                        <tr key={tx.id} className={`hover:bg-gray-50 transition-colors ${rowAccent} ${idx < filtered.length - 1 ? 'border-b border-gray-100' : ''}`}>
                                            ...
                                            <td className="px-4 py-3 text-sm whitespace-nowrap">
                                                {tx.payment_date
                                                    ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold bg-green-50 text-green-700 border border-green-100">✓ {fmtDate(tx.payment_date)}</span>
                                                    : <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-amber-50 text-amber-600 border border-amber-100">Unpaid</span>
                                                }
                                            </td>
                                ...
                                <tfoot className="bg-gray-50 border-t-2 border-gray-200">
                                    <tr>
                                        <td colSpan={3} className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">
                                            {t('bank.balance')}: <span className={`font-mono font-bold ml-1 ${balancePositive ? 'text-green-700' : 'text-red-700'}`}>
